"""Layer 2 sanitizer for mcp-safe-fetch.
...
cap. After every other strip pass, escapes literal ``<UNTRUSTED-*>``
sequences so an attacker can't break out of the envelope from inside.

Vendored from sharkyger/safe-fetch (MIT). Refresh from that project
when its sanitizer.py changes:

    cp ../safe-fetch/src/safe_fetch/sanitizer.py \\
       src/mcp_safe_fetch/sanitizer.py

Then re-apply this docstring header (everything above the ``from
__future__`` line). See NOTICE for the attribution chain back to the
original Tim Stark port.

Bench (2026-05-21, M2 Max via safe-fetch): 35.8 KB input -> 27 ms/call
-> 1317 KB/s on a representative HTML page with embedded zero-width
+ bidi noise. Roughly 5% of typical WebFetch network latency; safe to
leave on.
